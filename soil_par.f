...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, nly, ltn, jj, flag  !! ltn changed from n by qichun
      real ::  xx, plt_zmx, yy
      ...
      if (sol_zmx(i) > 0.001) then
        ...
        do j = 1, nly - 1
          ...
          if (sol_z(j,i) > sol_zmx(i)) then
            ...
            jj = j + 1
            do ltn = nly, jj, -1
              sol_z(ltn,i) = sol_z(ltn-1,i)
              sol_bd(ltn,i) = sol_bd(ltn-1,i)
              sol_awc(ltn,i) = sol_awc(ltn-1,i)
              sol_k(ltn,i) = sol_k(ltn-1,i)
              sol_cbn(ltn,i) = sol_cbn(ltn-1,i)
              sol_clay(ltn,i) = sol_clay(ltn-1,i)
              sol_no3(ltn,i) = sol_no3(ltn-1,i)
              sol_orgn(ltn,i) = sol_orgn(ltn-1,i)
              sol_orgp(ltn,i) = sol_orgp(ltn-1,i)
              sol_solp(ltn,i) = sol_solp(ltn-1,i)
            end do
            sol_z(j,i) = sol_zmx(i)
          end if
          if (flag == 1) exit

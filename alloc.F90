...
!! ... Function declarations
      ...
      real       rtimp                  !Calculate impact of root biomass on available nutrients
...
!! ... Use tree specific favail(1) value !Calculate soil available nutrients, based on a maximum fraction (favail)
      favail(1) = SFAVAIL2f(idf)

!! ... Estimate the fraction of carbon going to the roots
        ! ...   tfrtcn(1) - maximum fraction of C allocated to fine roots under maximum nutrient stress
        ! ...   tfrtcn(2) - minimum fraction of C allocated to fine roots with no nutrient stress
        ! ...   tfrtcw(1) - maximum fraction of C allocated to fine roots under maximum water stress
        ! ...   tfrtcw(2) - minimum fraction of C allocated to fine roots with no water stress
      fracrc = (TFRTCWf(idf,1)+TFRTCWf(idf,2)+TFRTCNf(idf,1)+TFRTCNf(idf,2))/4.0
...
!!  .. (favail) and the impact of root biomass (rimpct), adding storage.
      do 45 iel = 1, nelem
        eavail(iel) = (availm(j,iel) * favail(iel) * rimpct) + forstg(iel,j)
45    continue
     ...
     !! DEMAND based on the maximum E/C ratio or minimum C/E ratio.
     ...
     !! New calculation -mdh 5/10/01 available E to plant demand for E
        tree_a2drat(iel,j) = min(1.0, totale / demand)
        tree_a2drat(iel,j) = max(0.0, tree_a2drat(iel,j))
        ta2drat(iel) = tree_a2drat(iel,j)
50    continue

!!...... calculate the soil water impacts
           petdly = pet_day/10.0 !/10.0 to convert mm to cm h2o
   !! calculate tree water stress  !! find the wetest soil layer
      maxrwcf = -9999
      do 140 ii = 1, swatlyr
            rwcf(ii) = ((sol_st(ii,j)+sol_wpmm(ii,j)) *0.1) / (sol_thick(ii)*0.1)  !! relative wetness
...
140   continue
      ...
      twstress = twstress + min(1.0, maxrwcf)        
        ...
        if (petdly .ge. .01) then !the unit is cm
!! ....... Calculate potential growth based on the relative water content
!! ....... of the wettest soil layer, cak - 12/06/04
          h2ogef = 1.0/(1.0+exp(WSCOEFF2f(idf,2)*(WSCOEFF2f(idf,1)-twstress)))
        else
          h2ogef = 0.01
        endif
        !h2ogef-the effect of water on root to shoot ratio computed in POTCRP or POTFOR
        !a2drat(iel) - the ratio of available mineral to mineral demand by the plant

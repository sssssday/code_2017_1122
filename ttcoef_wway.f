...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !!implicit none
      integer :: jj, k
      real :: fps, d, b, ltp, a, qq1, rh, tt1, tt2, aa  !! ltp: little p, changed by qichun to avoid conflit with pulic variables
      real :: chsslope
      k = ihru
...
!!    compute flow and travel time at bankfull depth
      ...
      rh = 0.
      tt2 = 0.
      ltp = b + 2. * d * Sqrt(chsslope * chsslope + 1.)
      a = b * d + chsslope * d * d
      rh = a / ltp
      wat_phi(1,k) = a
...
!!    compute flow and travel time at 1.2 bankfull depth
      ...
      ltp=ltp + 4.*grwat_w(k) + (0.4 * grwat_d(k) * 
     &  Sqrt(fps * fps + 1.))
      rh = a / ltp

...
!!	Modified by N.Kannan, Blackland Research, Temple, USA
      ...
      real :: wtrin, c, ltp, scoef  !! ltp changed from p by Qichun
      real :: vol, topw
...
!!     start of sub-daily loop
     ...
     &        ch_w(2,jrch) * ch_w(2,jrch) / 64.) - 5.* ch_w(2,jrch) / 8.
        ...
        if (hdepth(ii) <= ch_d(jrch)) then
          ltp = phi(6,jrch) + 2. * hdepth(ii) * Sqrt(1. + c * c)
        else
          ltp = phi(6,jrch) + 2. * ch_d(jrch) * Sqrt(1. + c * c) + 4. * 
     &     ch_w(2,jrch) + 2. * (hdepth(ii) - ch_d(jrch)) * Sqrt(17.)
        ...
        if (hsdti(ii) > 0.) then
          ...
          hrttlc = 0.
          if (hhtime(ii) < 1.) then
            hrttlc(ii) = ch_k(2,jrch) * ch_l2(jrch) * ltp * hhtime(ii)
          else
            hrttlc(ii) = ch_k(2,jrch) * ch_l2(jrch) * ltp
          end if

...
!!    new arrays for routing units
      ...
      integer, dimension (:), allocatable :: pligst
...
!! qichun variables from daycent
      
      logical :: hrsinc
      logical :: decidgrow
      real :: aufert !! auto fertilizer use. This is a fraction used to ease the nutrient limitation fraction
      integer :: nelem
      real :: co2crs !! co2 effects on root
      real :: ffroota 
      integer :: growday !! days when plant grow
!! constant from daycent
...
!! add forest parameter here Qichun Yang 2015.8.12
!! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~  ~ ~ ~ ~ ~  ~ ~ ~ ~ ~  ~ ~ ~ ~ ~  ~ ~ ~ ~ ~  ~ ~ ~ ~ ~  ~ ~ ~ ~ ~  ~ ~ ~ ~ ~  ~ ~ ~ ~ ~
      character(len=4), dimension (:), allocatable ::cpnmfrst
      real, dimension (:), allocatable :: idcfrst
      real, dimension (:), allocatable ::salbdf
      real, dimension (:), allocatable ::slalbf
      real, dimension (:), allocatable ::slfgf
      real, dimension (:), allocatable ::omgaf
      real, dimension (:), allocatable ::inicdf
      real, dimension (:), allocatable ::avg_slaf 
      real, dimension (:), allocatable ::ext_nf
      real, dimension (:), allocatable ::fcn_leafminf
      real, dimension (:), allocatable :: raf 
      real, dimension (:), allocatable ::rbf
      real, dimension (:), allocatable :: ko25f
      real, dimension (:), allocatable ::kc25f
      real, dimension (:), allocatable ::vcmx25f 
      real, dimension (:), allocatable ::akof
      real, dimension (:), allocatable ::akcf
      real, dimension (:), allocatable ::avcmxf
      real, dimension (:), allocatable ::vpd_openf
      real, dimension (:), allocatable ::vpd_closef
      real, dimension (:), allocatable ::toptdf
      real, dimension (:), allocatable ::gmaxf
      real, dimension (:), allocatable ::gminf
      real, dimension (:), allocatable ::chtmaxf
      real, dimension (:), allocatable ::tbasef
      real, dimension (:), allocatable ::matyrsf
!!    variables from daycent    
        integer, dimension (:), allocatable ::DECIDf
        real, dimension (:),   allocatable ::  PRDX2f
        real, dimension (:,:), allocatable :: PPDFf
        real, dimension (:,:,:,:), allocatable :: CERFORf
        real, dimension (:,:), allocatable :: DECWf
        
        !! the following array read the third colum first
        real, dimension (:,:,:), allocatable :: FCFRACf
       
        real, dimension (:,:), allocatable :: TFRTCNf
        real, dimension (:,:), allocatable :: TFRTCWf
        real, dimension (:,:), allocatable :: LEAFDRf
        real, dimension (:),   allocatable ::  BTOLAIf
        real, dimension (:),   allocatable ::  KLAIf
        real, dimension (:),   allocatable ::  LAITOPf
        real, dimension (:),   allocatable ::  MAXLAIf
        real, dimension (:),   allocatable ::  MAXLDRf
        real, dimension (:,:), allocatable :: FORRTFf
        real, dimension (:),   allocatable ::  SAPKf
        real, dimension (:),   allocatable ::  SWOLDf
        real, dimension (:,:), allocatable :: WDLIGf
        real, dimension (:,:), allocatable :: WOODDRf
        real, dimension (:),   allocatable ::  WRDSRFCf
        real, dimension (:),   allocatable ::  WMRTFRACf
        real, dimension (:),   allocatable ::  SNFXMX2f
        real, dimension (:),   allocatable ::   DEL13Cf
        real, dimension (:),   allocatable ::  CO2IPR2f
        real, dimension (:),   allocatable ::  CO2ITR2f
        real, dimension (:,:,:), allocatable :: CO2ICE2f
        real, dimension (:),   allocatable ::  CO2IRS2f
        real, dimension (:),   allocatable ::  BASFC2f
        real, dimension (:),   allocatable ::  BASFCTf
        real, dimension (:),   allocatable ::  SITPOTf
        real, dimension (:),   allocatable ::  MAXNPf
        real, dimension (:,:), allocatable :: FKMRSPMXf
        real, dimension (:,:), allocatable :: FMRSPLAIf
        real, dimension (:,:), allocatable :: FGRESPf
        real, dimension (:),   allocatable ::  NO3PREF2f
        integer, dimension (:),   allocatable ::  TLAYPGf
        real, dimension (:),   allocatable ::  TMIXf
        real, dimension (:),   allocatable ::  TMPLFFf
        real, dimension (:),   allocatable ::  TMPLFSf
        integer, dimension (:),   allocatable ::  FURGDYSf
        real, dimension (:),   allocatable ::  FLSGRESf
        real, dimension (:),   allocatable ::  TMXTURNf
        real, dimension (:,:), allocatable :: WSCOEFF2f
        real, dimension (:),   allocatable ::  NPP2CS2f
        real, dimension (:),   allocatable ::  SFAVAIL2f
 
!!    biomass pools and fluxes added by Qichun Yang 2015. 8.19
      real, dimension (:), allocatable ::leafc, leafn, leafp
      real, dimension (:), allocatable ::brchc, brchn, brchp
      real, dimension (:), allocatable ::largwc, largwn, largwp
      real, dimension (:), allocatable ::frootjc, frootjn, frootjp
      real, dimension (:), allocatable ::frootmc, frootmn, frootmp 
      real, dimension (:), allocatable ::csrootc, csrootn, csrootp
      real, dimension (:), allocatable ::woodc1,woodc2
      real, dimension (:,:), allocatable ::woodc3
      real, dimension (:,:), allocatable :: eleaf, ebrch, elargw
      real, dimension (:,:), allocatable :: woode1, woode2
      real, dimension (:,:,:), allocatable :: woode3
      real, dimension (:,:), allocatable :: efrootj, efrootm
      real, dimension (:,:), allocatable :: ecsroot
      real, dimension (:), allocatable ::seedc, seedn, seedp
      real, dimension (:), allocatable ::flai,f_gpp, f_npp, f_pgpp
      real, dimension (:), allocatable ::f_mr, f_gr, inic
      real, dimension (:), allocatable ::f_moist, cal_temp
      real, dimension (:), allocatable ::csrsnk
      real, dimension (:,:), allocatable ::esrsnk
      real, dimension (:), allocatable ::carbostg
      real, dimension (:,:), allocatable :: forstg
      real, dimension (:,:), allocatable :: availm
      real, dimension (:,:,:), allocatable :: eup
      real, dimension (:), allocatable :: pwther
      
      
      real, dimension (:,:,:), allocatable :: co2cce
      real, dimension (:,:), allocatable :: tree_a2drat
      real, dimension (:,:,:), allocatable :: tavgdate !! average temperature of each hru with date information
      real, dimension (:), allocatable :: wood1tosom1
      real, dimension (:), allocatable :: wood1tosom2
      real, dimension (:), allocatable :: wood2tosom1
      ...
      integer::dofy !!current day of year    
...
!!    increased hedr(42) to hedr(45) for output.rch gsm 10/17/2011
      character(len=13) :: heds(110),hedb(22),hedr(46),hedrsv(41)

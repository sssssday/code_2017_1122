...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: ltn, l   !! changed from n by qichun
...
!!    set random number array values
      ...
      do ltn = 1, 3
        wgncur(ltn,j) = 0.
        do l = 1, 3
          wgncur(ltn,j) = wgncur(ltn,j) + b(ltn,l) * e(l)
          xx(ltn) = xx(ltn) + a(ltn,l) * wgnold(l,j)
        end do
      end do

      do ltn = 1, 3
        wgncur(ltn,j) = wgncur(ltn,j) + xx(ltn)
        wgnold(ltn,j) = wgncur(ltn,j)
      end do

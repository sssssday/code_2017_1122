  subroutine carbon_zhang2
      ...
      !!===========================================
       do k = 1, sol_nly(j)
          ...
          !! if (sol_tmp(k,j) > 0. .AND. sol_st(k,j) > 0.) then !! this is the original thresold
             if (sol_tmp(k,j) > -5.0 .AND. sol_st(k,j) > 0.) then !!! qichun
              !!from Armen
             ...
             fc_fp = 0.
             sat_fp = 1.
             t_scale = 4.89*exp(-3.432+0.1*sol_tmp(k,j)*(1-0.5*sol_tmp(k,j)/36.9)) 
     ...
     !! qichun code from century ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          !! water effects
            ...
            !!Check added to handle underflow potential in exp intrinsic
                  ...
                  rwcf = (wc/sol_z(k,j)-sol_wpmm(k,j)) / (fc-sol_wpmm(k,j))
                  if (rwcf > 13.) then
	                wcalc = 1.
                  else

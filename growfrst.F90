...
30    continue
      !UPPER CASE followed by "f" indicate read-in parameters  
        
      !UPPER CASE indicate Constant
      !ipart == 1 for , == 2 for 
...
50    continue
    ...
    !!! variable related to death rate multiplier
    ...
    !! ... Change leaf death rate multiplier if you have floating C/E ratios.
      if (ccefor(IMIN,LEAF,N) .ne. ccefor(IMAX,LEAF,N)) then
        if (leafc(j) .gt. 0) then
          lfncon = eleaf(N,j) / leafc(j)
          lfncmin = 1 / ccefor(IMIN,LEAF,N)
          lfncmax = 1 / ccefor(IMAX,LEAF,N)
          ldrmlt = 1 + (MAXLDRf(idf) - 1) * (lfncon - lfncmin) / (lfncmax - lfncmin)  !leaf death rate multiplier
          ldrmlt = max(0.,min(1.,fdif))
         endif
      endif
...
!!      in DLEM, fdif was first calculated with R, but then assigned with this values. need to check later
        fdif = 0.3                              !Fraction of diffuse
       
        fdif = max(0.,min(1.,fdif))
        fdir = 1.0 - fdif                       !fraction of direct
        swavgfd = hru_ra(j)*1.0e6/86400.0       !shortwave average flux density
...
!1        float sscater                   !! (W/m2) scattered solar radiation by sun leaf
        ...
        !avg_slaf(idf) specific LAI.  (LAI m2/ g C)
         !!if (lai>11.0)lai = 11.
       !!lai = MAXLAIf(idf) 
       ...
       print*, "lai=", lai
        if (lai < 0.0001) then
            ...
            eleaf(N,j) = leafc (j)/ fcn_leafminf(idf)
            eleaf(P,j) = eleaf(N,j) / 10. !! change later, need to set fcp ratio for each pft
        end if
...
!!          calculate wetness
        do lyr = 1, sol_nly(j)         
            soil_sand = soil_sand + sol_sand(lyr,j)
            soil_clay = soil_clay + sol_clay(lyr,j)
       ...
       do lyr = 2, sol_nly(j)        
            tmoist = tmoist + rdis(lyr,j)* sol_w(lyr)        
       end do
...
!!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    !! calculate the maintence reducing factor based on the size of carbon storage pool
        ! ... Linearly decrease maintenance respiration as the amount of
        ! ... carbohydrate stored in the carbohydrate storage pool gets
        ! ... smaller based on the carbon level that is required for optimal
        ! ... leaf area, cak - 10/20/2006
        ! ... We will be using two line functions for this calculation
        ! ... depending on the optimal leaf carbon value, cak - 08/13/2009
        ! ... Calculate the amount of carbon in the leaves if the tree is at
        ! ... its optimal LAI, cak - 10/20/2006  
        
       !! call lacalc(lai, fbrchc, rlwodc, maxlai, klai) we do not use this method since we calculate lai directly from leaf carbon
      rleavc_opt = lai / (2.5 * BTOLAIf(idf))    !optimal amount of carbonin the leaves if the tree is at its optimal LAI
...
!!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~       
  !! maintenance respiration from all tissues   
        ...
        !! ids for different plant tisses
         ...
         mrleaf = tfrac *leafc(j)*mrspReduce*mrspteff* FKMRSPMXf(idf,1)                 !rleavc     - tree leaf carbon (gC/m^2)
   ...
   !! reduce this from the carbon storage pool and added to csrsnk pool
    ...
    !! get the alloction fraction for each tissuse
      ...
      cdlength  = daylenth (cday, j) !current day length         
      pdlength = daylenth (pday, j)  !previous day length
          
        if (cdlength .lt. pdlength) then
            hrsinc = .FALSE.                        !hours increase false
        else if (cdlength .gt. pdlength) then
            hrsinc = .TRUE.                         !hours increase true
        endif
       ...
       checkgrow = grochk(hrsinc,pdlength,tmpav(j))
        ...
        call alloc (f_pgpp(j), tmpav(j), tlaypgswat,tree_cfrac, checkgrow) !tree_cfrac 
        riint = 0.8 !
        rictrl = 0.015 !
      riint = 0.8                           !! root impact intercept used by rtimp; used for calculating the impact of root biomass on nutrient availability
      rictrl = 0.015                        !! root impact control term used by rtimp; used for calculating the impact of root biomass on nutrient availability
    ...
    !! XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXcheck should calculate f_gpp based on f_pgpp here   because in daycent actural gpp is calcualted twic, consider to further change this later
            ...
            !!... minimum, and maximum whole plant nutrient concentrations.
               ...
               call restrp(elimit, nelem, ccefor, 2, tree_cfrac,        &
                           f_pgpp(j), rimpct, forstg, SNFXMX2f(idf), cprodfdy,     &
                           eprodfdy, uptake,  treeNfix, relyld)
...
!!          STOP
    ...
    !! XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
     ...
     !! limit potential gpp again here
      call restrp(elimit, nelem, ccefor, FPARTS-1,                &
                  tree_cfrac, f_pgpp(j), rimpct, forstg, SNFXMX2f(idf), & 
                  cprodfdy, eprodfdy, uptake, treeNfix,    &
                  relyld)
...
!! ..... New variable MFPRD added for gridded output - 6/96 rm
        do 95 ipart = 1, 6
          if (ipart .eq. LEAF .or. ipart .eq. FBRCH .or.      &
             ...
             !!print*,"gpp=", f_gpp(j), "leaffraction=", tree_cfrac(LEAF)
...
!!     calculate growth respiration
 ...
 !!      add carbon to tissuse pools a
         ...
         if ((DECIDf(idf).eq. 1) .and. (growday .gt. FURGDYSf(idf))) then !furgdys     - number of days after leaf out of unrestricted growth of woody tree components for a decidious tree type
             mfprd(FBRCH)= mfprd(FBRCH)* (1-FLSGRESf(idf))                      !flsgres     - late season forest growth restriction factor(0.0 - 1.0)
...
!!    XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX  
    ...
    !! ..... Actual Uptake
        do 110 iel = 1, nelem
          ...
          euf(LEAF) = eup(LEAF,iel,j) / eprodfdy(iel)
          euf(FROOTJ) = (eup(FROOT,iel,j) * (1.0 - WMRTFRACf(idf))) /   &         !WMRTFRAC is the portion to Mature fine root
                       eprodfdy(iel)
          euf(FROOTM) = (eup(FROOT,iel,j) * WMRTFRACf(idf)) / eprodfdy(iel)
          euf(FBRCH) = eup(FBRCH,iel,j) / eprodfdy(iel)
          euf(LWOOD) = eup(LWOOD,iel,j) / eprodfdy(iel)
          euf(CROOT) = eup(CROOT,iel,j) / eprodfdy(iel)
...
!! ....... the woody components, cak - 03/11/2010
            ...
            !!if(forstg(iel,j)<0.001)  forstg(iel,j) = 0.001
            !!call nfix
           !! print*, "fixed N = ", fixn
          if (iel .eq. N ) forstg(iel,j) = forstg(iel,j) + fixn
        
          if (forstg(iel,j) .gt. 0.0) then
            amt = uptake(ESTOR,iel) * euf(LEAF)
            forstg(iel,j) = forstg(iel,j) - amt
            eleaf(iel,j) = eleaf(iel,j) + amt
            ...
            forstg(iel,j) = forstg(iel,j) - amt
            efrootj(iel,j) = efrootj(iel,j) + amt
            ...
            forstg(iel,j) = forstg(iel,j) - amt
            efrootm(iel,j) = efrootm(iel,j) + amt
            !!eupprt(FROOT,iel) = eupprt(FROOT,iel) + amt
            !!eupacc(iel) = eupacc(iel) + amt
            eprodfdy(iel) = eprodfdy(iel) + amt
            ...
            if ((DECIDf(idf).eq. 1) .and. (growday .gt. FURGDYSf(idf))) then  !furgdys     - number of days after leaf out of unrestricted growth of woody tree components for a decidious tree type
              ...
              forstg(iel,j) = forstg(iel,j) - amt
              ecsroot(iel,j) = ecsroot(iel,j) + amt
...
!!              fertac(iel) = fertac(iel) + uptake(EFERT,iel) + amt
    ...
    !!XXXXXXX             lyr = SRFC
     ...
     !!XXXXXXX                             frac_no3_fert, ammonium, nitrate, &
      !!XXXXXXX                            subname)
       !!XXXXXXX        endif
 !! XXXXX              call flow(esrsnk(iel,j),minerl(SRFC,iel),time,amt)  !! check check
         endif
         endif
110    continue
...
!!XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
  !! calcuate the average temperature of the past 7 days (including current day)
     ...
     tavgp7d =0.
     tavgp7d = tavgp7d + tavgdate(curyr,(dofy),j)
  
     do dd = 1, 6
         if ((dofy-dd ) .gt.0) then     
            tavgp7d = tavgp7d + tavgdate(curyr,(dofy-dd),j)
            cdd = cdd + 1
         else if (curyr > 1) then
            tavgp7d = tavgp7d + tavgdate((curyr-1),(dofy-dd+yeardays),j)
            cdd = cdd + 1
         end if
     end do

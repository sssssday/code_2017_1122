...
!!    Spatial Sciences Laboratory, Texas A&M University
      ...
      !!implicit none
      integer :: jrch
      real :: wtrin, scoef, ltp, tbase, topw, vol, c, rh   !! ltp changed from p in the original version by Qichun
...
!! Find maximum flow capacity of the channel at bank full
      c = 0.
      c = chside(jrch)
	ltp = phi(6,jrch) + 2. * ch_d(jrch) * Sqrt(1. + c * c)
	rh = phi(1,jrch) / ltp
...
!! then simulate flood plain flow else simulate the regular channel flow
	...
	!! find the depth until the discharge rate is equal to volrt
	  Do While (sdti < volrt)
          ...
          addp = ltp + (ch_w(2,jrch) * 4) + 2. * adddep 
     &     * Sqrt(1. + 4 * 4)
	    rh = addarea / addp
	...
	!! find the depth until the discharge rate is equal to volrt
	  Do While (sdti < volrt)
	    ...
	    ltp = phi(6,jrch) + 2. * rchdep * Sqrt(1. + c * c)
	    rh = rcharea / ltp
...
!! channel storage and from volume flowing out
	...
	!!  Total time in hours to clear the water
          ...
          rttlc = det * ch_k(2,jrch) * ch_l2(jrch) * ltp

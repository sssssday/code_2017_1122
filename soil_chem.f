...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use parm
      !!implicit none
      integer :: nly, j, jj, ltn  !! ltn changed from n by qichun
      ...
      do j = 1, npmx
        ...
        if (jj > 0) then
          ...
          do ltn = 1, nly
            ...
            dg = (sol_z(ltn,i) - xx)
            xx = sol_z(ltn,i) 
            wt1 = sol_bd(ltn,i) * dg / 100.              !! mg/kg => kg/ha
            sol_kp(j,i,ltn) = skoc(jj) * sol_cbn(ltn,i) / 100.
            sol_pst(j,i,ltn) = solpst * wt1
          end do
        end if
      end do
      end if
